...
function afterSalesAttachmentLinks(row){
  ...
  return `<span style="display:inline-flex;flex-wrap:wrap;gap:6px;align-items:center">${files.map((f,i)=>{
    const token=f?.file_token||f?.token||'', name=f?.name||f?.file_name||`Attachment ${i+1}`;
    if(!token) return `<span class="btn-light" style="display:inline-block;padding:4px 8px;white-space:nowrap">📎 ${esc(name)}</span>`;
    ...
    return `<a class="btn-light" style="display:inline-block;padding:4px 8px;white-space:nowrap" href="/api/download-lark-attachment?${q.toString()}" target="_blank" rel="noopener">📎 ${esc(name)}</a>`;
  }).join('')}</span>`;
}
