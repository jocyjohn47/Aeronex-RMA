...
async function runDealerRepairDiagnostics(){
  const el=$('dealerRepairDiagResult');
  if(el) el.textContent='Running Dealer Repair Case diagnostics...';
  try{
    const d=await api('/api/admin-dealer-repair-diagnostics?role='+encodeURIComponent(S.user.role||''));
...
function renderAdmin(){
  ...
  if(isAdmin() && $('admin')){
    $('admin').innerHTML += `<div class="panel"><h2>Dealer Repair Case Diagnostics</h2><div class="notice">Admin-only check for Lark table access. Use this only for troubleshooting.</div><button onclick="runDealerRepairDiagnostics()">Run Dealer Repair Diagnostics</button><pre id="dealerRepairDiagResult" style="white-space:pre-wrap;background:#f6f8fb;border:1px solid #dbe3ef;border-radius:10px;padding:12px;max-height:360px;overflow:auto"></pre></div>`;
  }
}
