...
function backupAccessAllowed(role) {
  const r = lower(role);
  return r.includes("admin") || r.includes("technician") || r.includes("technicain") || r.includes("techncian") || r.includes("tech");
}
...
async function handle(req, env) {
  ...
  if (p === "/api/backup-status") {
    const role = norm(url.searchParams.get("role") || req.headers.get("x-aeronex-role") || "");
